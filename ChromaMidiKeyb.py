...
Info.Description = "RaddedMC's MIDI light-show plugin for Razer Chroma. Designed for use with a Razer keyboard and Ableton Live. Source code at github.com/raddedmc/ChromaMidi ."
...
print("Note: This version of ChromaMidi currently only supports Chroma-enabled keyboards. Other devices can be added on request. Open the Chroma Connect app or PM u/RaddedMC if you're interested in support for other devices.")
print("---------------------------------------")

#light test
print("The keyboard has " + str(len(KeyboardGrid)) + " lights.")
...
print("Testing keyboard lights...")

for i in range(0, len(KeyboardGrid)):
    for j in range(0, len(KeyboardGrid[i])):
        # RED
        print("Light [" + str(i) + "][" + str(j) + "]: ON")
        KeyboardGrid[i][j].set(red=255,green=0,blue=0)
        App.Keyboard.setCustomGrid(KeyboardGrid)
        App.Keyboard.applyGrid()
        sleep(0.01)
        
        # GREEN
        KeyboardGrid[i][j].set(red=0,green=255,blue=0)
        App.Keyboard.setCustomGrid(KeyboardGrid)
        App.Keyboard.applyGrid()
        sleep(0.01)
        
        # BLUE
        KeyboardGrid[i][j].set(red=0,green=0,blue=255)
        App.Keyboard.setCustomGrid(KeyboardGrid)
        App.Keyboard.applyGrid()
        sleep(0.01)
...
def setALight(lightToSet, lightR, lightG, lightB):
    try:
        KeyboardGrid[math.floor(lightToSet/21)][abs(lightToSet%21)].set(red=lightR, green=lightG, blue=lightB)
        App.Keyboard.setCustomGrid(KeyboardGrid)
        App.Keyboard.applyGrid()
    except IndexError:
